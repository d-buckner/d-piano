...
export interface PianoSamplerOptions extends ToneAudioNodeOptions {
	/**
...
/**
 * A fully-loaded piano sampler at a specific velocity resolution.
 * Internal building block used by Piano for progressive loading.
 */
export class PianoSampler extends ToneAudioNode<PianoSamplerOptions> {

  readonly name = 'PianoSampler';
  ...
  constructor(options?: Partial<PianoSamplerOptions>);

  constructor() {
    super(optionsFromArguments(PianoSampler.getDefaults(), arguments));

    const options = optionsFromArguments(PianoSampler.getDefaults(), arguments);
  ...
  static getDefaults(): PianoSamplerOptions {
   ...
   */
  async expandTo(velocities: number): Promise<void> {
    await this._strings.expandTo(velocities);
  }
  ...
  pedalUp({ time = this.immediate() }: PedalEvent = {}): this {
    ...
    if (this.loaded) {
      ...
      if (this._pedal.isDown(seconds)) {
        ...
        this._sustainedNotes.forEach((_t, note) => {
